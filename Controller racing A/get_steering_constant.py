...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            world.player.apply_control(_control)
            # v = world.player.get_velocity()
            omega = world.player.get_angular_velocity().z
            t = world.player.get_transform()
            v = world.player.get_velocity()
            angle_heading = t.rotation.yaw * pi/ 180
            vx = v.x
            vy = v.y
            curr_speed_x = vx*math.cos(angle_heading) + vy*math.sin(angle_heading)
            steers_time.append([hud.simulation_time,omega,curr_speed_x])
            pygame.display.flip()

        store_trajectory_plot(trajectory_fig.fig, 'trajectory.png')
        # np.savetxt('outputs/comp_times.csv',np.array(time_estimates))
        np.savetxt('outputs/steer_const.csv',np.array(steers_time))
