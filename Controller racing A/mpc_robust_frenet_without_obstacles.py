...
if True :
    ...
    save_path_after = 370 # Start saving path after this much time from start (in s)
...
if True : 
    ...
    solver=nlpsol("solver","blocksqp",nlp_prob,options)
...
def game_loop(args):
    ...
    try:
        ...
        debug_helper = client.get_world().debug
        # debug_helper.draw_line(carla.Location(-69.4,238,1),carla.Location(-69.4,254,1),thickness=1.,color=carla.Color(0,255,0))
        # debug_helper.draw_string(carla.Location(-69.4,238,2),'Start line',color=carla.Color(255,120,0))
        # debug_helper.draw_line(carla.Location(-16,152.6,1),carla.Location(-0,152.6,1),thickness=1.,color=carla.Color(0,255,0))
        # debug_helper.draw_string(carla.Location(-16,152.6,2),'End line',color=carla.Color(255,0,0))
        while True:
        ...
        trajectory_fig.add_graph("left_boundary", window_size=len(ty_center),
                                 x0=tx_center-5.5*np.sin(tyaw_center), y0=ty_center+5.5*np.cos(tyaw_center),
        ...
        while True:
            ...
            if True :
                ...
                ref_poses[:,:2] = 0
                p=list(current_pose)+list(ref_poses[:N,:3].reshape(3*N))+[a_eqn,b_eqn,c_eqn]
                ...
                errs = errors_e1[min(int(abs(curvature)//0.018),2)]
                i = 0
        ...
        names_to_save = ['trajectory','left_boundary','right_boundary']
