...
if True :
    ...
    class World(object):
        ...
        def restart(self):
            ...
            rear_left_wheel   = carla.WheelPhysicsControl(tire_friction=1.5, damping_rate=1.5, max_steer_angle=0.0)#,  long_stiff_value=1000)
            rear_right_wheel  = carla.WheelPhysicsControl(tire_friction=1.5, damping_rate=1.5, max_steer_angle=0.0)#,  long_stiff_value=1000)
            wheels = [front_left_wheel, front_right_wheel, rear_left_wheel, rear_right_wheel]
            physics_control = vehicle.get_physics_control()
            # physics_control.wheels[0].tire_frictio=1
...
if True :
    ...
    Td = 0.02 # Actuator processing delay (in s)
...
if True :
    ...
    def get_future_state_est(current_pose,buff_con,curr_time_est) :
        ...
        itr = 0
        while (curr_time_est > T and itr<6) :
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            if True : 
                global curr_x,curr_y,curr_speed_x,curr_speed_y, last_speed_y, last_time_t, not_turned
                curr_x = px
                curr_y = py
                ...
                c_speed = speed #min((curr_speed_x**2 + curr_speed_y**2)**0.5+1,10)  # current speed [m/s]
                _,near_point_stats = get_path(px, py, angle_heading, trajectory_to_follow.tolist(), int(2*N), speed, T)
                # TO BE CHANGED
                a,b,c,s0_d, s0 = near_point_stats[0],near_point_stats[1],near_point_stats[2],near_point_stats[3],near_point_stats[4]
                print("Got s0 as ", s0_d, s0)
                # yaw_diff = -math.atan2(-a,b)
                c_d = c/((a**2+b**2)**(1/2)) # current lateral position [m]
                c_d_d = 0#math.sqrt(vx**2 + vy**2)*sin(yaw_diff)  # current lateral speed [m/s]
                ...
                curvature = 0
                print(curvature)
                e2_ = world.player.get_angular_velocity().z - curr_speed_x*curvature
                # s0 = curr_x - tx[0]  # current course position
                _,near_point_stats = get_path(px, py, angle_heading, centre_line.tolist(), int(2*N), speed, T)
                # TO BE CHANGED
                a,b,c,s0 = near_point_stats[0],near_point_stats[1],near_point_stats[2],near_point_stats[3]
                c_d_center = c/((a**2+b**2)**(1/2))
                ob = np.array([
                            [x_obst, y_obst],
                            ])
                ob_speed = np.array([
                ...
                out_data.append([world.imu_sensor.accelerometer[1],curr_speed_y,world.player.get_angular_velocity().z,curr_speed_x])
                print("To frenet planner : ", s0, c_speed, c_d, c_d_d, c_d_dd, ob, ob_speed)
                path = frenet_optimal_planning(
                        csp, s0, c_speed, c_d, c_d_d, c_d_dd, ob, ob_speed,(c_d_center-c_d))
            ...
            if True :
                ...
                trajectory_fig.roll("car_shifted", current_pose[0], current_pose[1])
                ...
                trajectory_fig.update("ref_path", 
                        ref_poses[:, 0],
                        ref_poses[:, 1],
                ...
                curr_time_est = min(curr_time_est,0.1)
                ...
                # msg.brake = float(-min(buff_con[0,0],0))
            ...
            world.render(display)
            pygame.display.flip()
            np.savetxt('outputs/stiff_coeff.csv',out_data)
