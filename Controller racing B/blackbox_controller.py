...
import time
import lqr_steering_adjust_frenet as lqrs
...
def blackbox_controller(path,curr_pos) :
    ...
    rotation = curr_pos[2] + (inc)*(math.atan2(curr_pos[4],curr_pos[3]))
    dx = dx_*cos(rotation) + dy_*sin(rotation)
    dy = dy_*cos(rotation) - dx_*sin(rotation)
...
def get_future_state_est(current_pose,buff_con,curr_time_est) :
    if COMPENSATE_COMP_DELAY :
        # curr_time_est += 0.1
        return np.array(current_pose) + np.array([current_pose[3]*cos(current_pose[2])-current_pose[4]*sin(current_pose[2]),current_pose[3]*sin(current_pose[2])+current_pose[4]*cos(current_pose[2]),current_pose[5],0,0,0,0])*curr_time_est/factor
    else :
    ...
    while True:
        ...
        for sample in input_speed.samples.valid_data_iter:
            ...
            if COMPENSATE_ACTUATOR_DELAY :
                curr_steering_array = np.array(lqrs.get_optimal_commands(all_vehicles, curr_steering_array,curr_steering,curr_pos[-3],curr_pos[-2],curr_pos[-1],centre_line_eq,[curr_mu,diff_torque,lr_ratio,gyl,gyr,diff_f,diff_r],radius))
            print("Finished here")
